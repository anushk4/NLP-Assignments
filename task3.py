...
###############################################################################
# PreTrainedWordPieceTokenizer: loads existing vocab instead of constructing anew
###############################################################################
class PreTrainedWordPieceTokenizer(WordPieceTokenizer):
    """
    A subclass of WordPieceTokenizer that loads an existing vocabulary file
    (e.g. "vocabulary_100000.txt") instead of constructing it from a corpus.
    """
    def __init__(self, vocab_file_path):
        # Bypass the original corpus/vocab_size
        super().__init__(corpus=None, vocab_size=None)
    ...
    def load_existing_vocab(self, vocab_file_path):
        """Load the previously saved WordPiece vocabulary from a file."""
        with open(vocab_file_path, "r", encoding="utf-8") as vf:
            lines = vf.read().splitlines()
        self.vocab = lines
        print(f"Loaded {len(self.vocab)} tokens from {vocab_file_path}.")

    def tokenize(self, sentence):
        """Tokenize a sentence using the pre-loaded vocabulary (no new merges)."""
        import re
        # Basic preprocessing
    ...
    def encode_word(self, word):
        """Attempt to match largest subword in self.vocab; else [UNK]."""
        tokens = []
        while len(word) > 0:
            i = len(word)
...
# Training & Utility
###############################################################################
def train_model(model, train_loader, valid_loader, num_epochs=5, lr=1e-3, device='cpu'):
    model.to(device)
...
if __name__ == "__main__":
    ...
    with open("idx_to_word.json", "r", encoding="utf-8") as f:
    ...
    print("\n=== Training NeuralLM1 ===")
    res1 = train_model(model1, train_loader, valid_loader,
                       num_epochs=EPOCHS, lr=LEARNING_RATE, device=DEVICE)
    results["model1"] = res1

    print("\n=== Training NeuralLM2 ===")
    res2 = train_model(model2, train_loader, valid_loader,
                       num_epochs=EPOCHS, lr=LEARNING_RATE, device=DEVICE)
    results["model2"] = res2

    print("\n=== Training NeuralLM3 ===")
    res3 = train_model(model3, train_loader, valid_loader,
                       num_epochs=EPOCHS, lr=LEARNING_RATE, device=DEVICE)
    results["model3"] = res3
    ...
    if os.path.exists(test_file):
        ...
        best_model = model3  # or whichever
